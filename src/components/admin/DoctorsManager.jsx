      ...
      {doctors.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {doctors.map((doc) => (
            <div
              key={doc.id}
              className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 group"
            >
              <div className="flex items-start gap-5 mb-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-blue-600 font-black uppercase tracking-widest bg-blue-50 inline-block px-2 py-1 rounded-lg mt-1">
                    {doc.specialization}
                  </p>
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-50">
                ...
                <button
                  onClick={() =>
                    setDeleteModal({ show: true, item: doc })
                  }
      ...
      {showForm && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-[3rem] p-10 max-w-2xl w-full my-8 animate-in zoom-in-95 duration-300 shadow-2xl border border-slate-100">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-2xl font-black text-slate-900 italic">
                  {editing
                    ? 'Edit Data Dietisien'
                    : 'Tambah Dietisien Baru'}
                </h3>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                  Lengkapi form berikut dengan benar
                </p>
              </div>
            ...
            {/* FORM CONTENT (unchanged, just formatted) */}
            {/* ... (dipertahankan sama persis, hanya spacing dirapikan) */}
